...
class EditProfileController: UIViewController {
    ...
    func loadUserData() {
        ...
        guard let img = self.user?.profileImageUrl else { return }
        profileImageView.loadImage(with: img)
    ...
    func updateProfileImage() {
        ...
        guard let currentUid = Auth.auth().currentUser?.uid else { return }
        //guard let user = self.user else { return }
        if let img = self.user?.profileImageUrl {
            Storage.storage().reference(forURL: img).delete(completion: nil)
        }
        ...
        STORAGE_PROFILE_IMAGES_REF.child(filename).putData(imageData, metadata: nil) { (metadata, error) in
            ...
            STORAGE_PROFILE_IMAGES_REF.child(metadata.name!).downloadURL(completion: { (url, error) in
